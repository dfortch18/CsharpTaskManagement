...
namespace CsharpTaskManagement.Cli
{
    public class CsharpTaskManagementCli
    {
        private readonly ITaskRepository _taskRepository;

        public CsharpTaskManagementCli(ITaskRepository taskRepository)
        {
        ...
        public void Start(string[] args)
        {
            Parser.Default.ParseArguments<AddOptions, ListOptions, GetOptions, MarkDoneOptions, DeleteOptions>(args)
                .MapResult(
                    (AddOptions opts) => AddTask(_taskRepository, opts),
                    (ListOptions opts) => ListTasks(_taskRepository, opts),
                    (GetOptions opts) => GetTask(_taskRepository, opts),
                    (MarkDoneOptions opts) => MarkDoneTask(_taskRepository, opts),
                    (DeleteOptions opts) => DeleteTask(_taskRepository, opts),
                    errs => 1
                );
        }
    ...
    [Verb("list", HelpText = "List the tasks")]
